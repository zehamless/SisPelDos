<div class="flex flex-col h-full space-y-4 relative" wire:poll.5s>
    <div class="flex-grow overflow-y-auto" id="comments-container">
        @if (count($comments))
            <x-filament::grid class="gap-4">
                @foreach ($comments->reverse() as $comment)
                    ...
                    @endphp
                    <div class="fi-in-repeatable-item block rounded-xl bg-gray-50 p-4 shadow-sm ring-1 ring-gray-950/5 dark:bg-white/5 dark:ring-white/10 {{$isMine ? "ml-auto":""}}">
    ...
    @if (auth()->user()->can('create', \Parallax\FilamentComments\Models\FilamentComment::class))
        <div class="sticky bottom-0 bg-white dark:bg-gray-800 p-4">
            <div class="mb-4">
                {{ $this->form }}
            </div>
